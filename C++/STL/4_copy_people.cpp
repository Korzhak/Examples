...
int main() {
    ...
    vector<Person> people_2;
    copy(people.begin(), people.end(), back_inserter(people_2));

    ShowPeople(people_2, true);


    vector<Person> people_3;
    copy_if(people.begin(), people.end(), back_inserter(people_3), [](Person p) {
        return p.GetAge() > 18;
    });

    ShowPeople(people_3, true);


    vector<Person> people_4;
    copy_if(people.begin(), people.end(), back_inserter(people_4), [](Person p) {
        return p.GetName() == "Taras";
    });

    ShowPeople(people_4, true);
    return 0;
}